...
export function TimerOverlay() {
  ...
  return (
    <div className={`timer-overlay-shell ${isTopAnchor ? 'items-start' : 'items-end'}`}>
      <div
        className={`timer-overlay-card group ${isTopAnchor ? 'origin-top' : 'origin-bottom'} ${isDragging ? 'is-dragging' : ''}`}
        onPointerDown={startDrag}
        onPointerUp={stopDrag}
        onPointerCancel={stopDrag}
        onDoubleClick={openMainWindow}
      >
        <div className={`flex ${isTopAnchor ? 'flex-col' : 'flex-col-reverse'}`}>
          <div className="relative flex h-[54px] items-center gap-3 rounded-[20px] border border-white/12 bg-[#101010]/76 px-4 shadow-[0_18px_55px_rgba(0,0,0,0.34)] backdrop-blur-2xl">
          ...
          <div className={`pointer-events-none flex justify-end gap-2 px-2 opacity-0 transition-all duration-200 group-hover:pointer-events-auto group-hover:opacity-100 ${isTopAnchor ? 'translate-y-[-8px] pt-2 group-hover:translate-y-0' : 'translate-y-[8px] pb-2 group-hover:translate-y-0'}`}>
            <button
              type="button"
